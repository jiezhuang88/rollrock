...
jQuery.fn.slideViewerPro = function(settings) {
  ...
  return this.each(function(){
    ...
    container.append('<span class="s-left J_left'+j+'">left</span>');
    container.append('<span class="s-right J_right'+j+'">right</span>');
    /**
    ...
    */
    ...
    container.each(function(i) {
      ...
      jQuery('.J_right' + j).bind("click", function(){
        var ul = container.find("ul"), li, cnt, z;
        li = ul.find('li');
        z= Math.floor(Math.abs(parseInt(ul.css('left'), 10) / 500) + 1);
        if( z >= li.length ){
          z = li.length - 1;
        }
        jQuery(this).find("p.tmbrdr").css({borderColor: settings.thumbsActiveBorderColor, opacity: settings.thumbsActiveBorderOpacity});
        ...
        cnt = -(pictWidth*z);
        (cnt != container.find("ul").css("left").replace(/px/, "")) ? container.find("span.typo").animate({"opacity": 0}, 250) : null ;
        ul.animate({ left: cnt}, settings.easeTime, settings.easeFunc, function(){container.find("span.typo").animate({"opacity": settings.typoFullOpacity}, 250)});          
        return false;
      });

      jQuery('.J_left' + j).bind("click", function(){
        var ul = container.find("ul"), li, cnt, z;
        li = ul.find('li');
        z= Math.floor(Math.abs(parseInt(ul.css('left'), 10) / 500) - 1);
        if( z <= 0 ){
      ...
      if(settings.autoslide){          
          ...
          jQuery(".J_right" + j).bind("mouseup", function(){
            jQuery(this).parent().parent().find('.thumbSlider').stopTime("asld");
          });
          jQuery(".J_left" + j).bind("mouseup", function(){
            jQuery(this).parent().parent().find('.thumbSlider').stopTime("asld");
          });
